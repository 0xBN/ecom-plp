const RemoveItemSVG = () => {
  return (
    // <svg
    // xmlns='http://www.w3.org/2000/svg'
    // viewBox='0 0 24 24'
    // className='h-10 w-10 md:w-10 md:h-10 hover:opacity-60 text-red-800'
    // fill='currentColor'
    // stroke='none'
    // >
    //   <path d='M19,3H5A2,2 0 0,0 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V5A2,2 0 0,0 19,3M19,19H5V5H19V19M17,8.4L13.4,12L17,15.6L15.6,17L12,13.4L8.4,17L7,15.6L10.6,12L7,8.4L8.4,7L12,10.6L15.6,7L17,8.4Z' />
    // </svg>
    // <svg
    // xmlns='http://www.w3.org/2000/svg'
    // viewBox='0 0 24 24'
    // className='h-10 w-10 md:w-10 md:h-10 hover:opacity-60 text-red-500'
    // fill='currentColor'
    // stroke='none'
    // >
    //   <path d='M9,3V4H4V6H5V19A2,2 0 0,0 7,21H17A2,2 0 0,0 19,19V6H20V4H15V3H9M7,6H17V19H7V6M9,8V17H11V8H9M13,8V17H15V8H13Z' />
    // </svg>
    <svg
      xmlns='http://www.w3.org/2000/svg'
      viewBox='0 0 24 24'
      className='h-10 w-10 md:w-10 md:h-10 hover:opacity-60 text-red-600'
      fill='currentColor'
      stroke='none'
    >
      <path d='M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z' />
    </svg>
  );
};
export default RemoveItemSVG;
